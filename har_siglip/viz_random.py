...
from .evaluate import _get_class_names_from_split_root  # reuse helper


def _build_text_matrix(model: SigLIPModel, class_names: List[str]):
    prompts = [f"This is a photo of {c.replace('_',' ')}." for c in class_names]
    return model.text_features(prompts)  # (C, D)


def predict_single(model: SigLIPModel, image_path: str, class_names: List[str]):
...
def main():
    ...
    class_names = _get_class_names_from_split_root(data_root)
    # pick a random image
    c = random.choice(class_names)
    cdir = os.path.join(data_root, c)
    img_files = [f for f in os.listdir(cdir) if f.lower().endswith((".jpg", ".jpeg", ".png"))]
    image_path = os.path.join(cdir, random.choice(img_files))

    # model
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = SigLIPModel(SigLIPCfg(name=cfg["model"]["name"], device=device))
    ...
    out_path = os.path.join(out_dir, "viz_random_siglip.png")
